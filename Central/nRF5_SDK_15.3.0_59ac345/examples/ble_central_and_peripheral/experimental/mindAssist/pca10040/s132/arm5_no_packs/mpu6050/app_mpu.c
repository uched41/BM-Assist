...
accel_values_t myAccelValues;
gyro_values_t myGyroValues;
...
uint32_t app_mpu_motion_detect_threshold(uint8_t val)
{
		nrf_drv_mpu_write_single_register(0x20, 10);
    return nrf_drv_mpu_write_single_register(MPU_REG_MOT_THR, val);
...
uint32_t app_mpu_init(void)
{
    ...
    if(err_code != NRF_SUCCESS) return err_code;
		...
		uint8_t pd;
		err_code = nrf_drv_mpu_read_registers(MPU_REG_WHO_AM_I, &pd, 1);
		if(err_code != NRF_SUCCESS) return err_code;
