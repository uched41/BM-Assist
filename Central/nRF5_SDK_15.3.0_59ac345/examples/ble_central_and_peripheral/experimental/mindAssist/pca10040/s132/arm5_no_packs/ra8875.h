...
typedef struct {
	uint8_t _cs, _rst, _mosi, _miso, _sck;
	const nrf_drv_spi_t* _spi;
  uint16_t _width, _height;
  uint8_t _textScale;
  enum RA8875sizes _size;
}ra8875_t;

extern ra8875_t display;
...
void    pushPixels(ra8875_t* disp, uint32_t num, uint16_t p);
void    fillRect(ra8875_t* disp);

/* Adafruit_GFX functions */
void    drawPixel(ra8875_t* disp, int16_t x, int16_t y, uint16_t color);
void    drawPixels(ra8875_t* disp, uint16_t * p, uint32_t count, int16_t x, int16_t y);
...
bool waitPoll(ra8875_t* disp, uint8_t r, uint8_t f);
uint16_t width(ra8875_t* disp);
uint16_t height(ra8875_t* disp);

/* Play nice with Arduino's Print class */
static size_t write(ra8875_t* disp, uint8_t b) {
	textWrite(disp, (const char *)&b, 1);
	return 1;
}

static size_t write_2(ra8875_t* disp, const uint8_t *buffer, size_t size) {
	textWrite(disp, (const char *)buffer, size);
	return size;
}
